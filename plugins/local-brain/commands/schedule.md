---
description: Install or remove local-brain scheduling (synthesize, reflect) via cron or launchd
---
...
- `/brain-schedule remove all` — remove all local-brain jobs
...
```bash
crontab -l 2>/dev/null | grep -n "local-brain\|/synthesize\|/brain-reflect" || echo "none"
```

On macOS, also check launchd:
```bash
ls ~/Library/LaunchAgents/com.local-brain.*.plist 2>/dev/null || echo "none"
```

### 2. If interactive (no arguments), show status and ask

Show a table of current state:
```
Current local-brain schedule:
...
**LOG** = `~/.claude/local-brain/cron.log`
...
```bash
(crontab -l 2>/dev/null; echo "<MINUTE> <HOUR> * * * <CLAUDE_PATH> -p \"/synthesize\" >> <LOG> 2>&1  # local-brain:synthesize") | crontab -
```

#### cron — install reflect

```bash
(crontab -l 2>/dev/null; echo "0 9 * * 1 <CLAUDE_PATH> -p \"/brain-reflect\" >> <LOG> 2>&1  # local-brain:reflect") | crontab -
```
...
Write `~/Library/LaunchAgents/com.local-brain.reflect.plist` with:
```xml
<key>StartCalendarInterval</key>
<dict>
...
```bash
crontab -l 2>/dev/null | grep -v "local-brain:synthesize" | crontab -
crontab -l 2>/dev/null | grep -v "local-brain:reflect" | crontab -
```

For `remove all`:
```bash
crontab -l 2>/dev/null | grep -v "local-brain:" | crontab -
```

#### launchd — remove

```bash
launchctl unload ~/Library/LaunchAgents/com.local-brain.synthesize.plist 2>/dev/null
rm -f ~/Library/LaunchAgents/com.local-brain.synthesize.plist

launchctl unload ~/Library/LaunchAgents/com.local-brain.reflect.plist 2>/dev/null
rm -f ~/Library/LaunchAgents/com.local-brain.reflect.plist
```
...
✓ Removed: synthesize, reflect
  No local-brain jobs scheduled on this machine.
```

If role is `source`, warn:
> "Note: this machine's role is 'source'. Scheduling synthesis here is unusual — are you sure? Consider running /brain-role to switch to 'aggregator' or 'standalone'."
