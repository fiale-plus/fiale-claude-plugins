...
Otherwise (activate):
- Run: `[ -f ~/.claude/status-config.json ] || echo '{}' > ~/.claude/status-config.json && jq '.remoteLayout = true' ~/.claude/status-config.json > /tmp/rl.json && mv /tmp/rl.json ~/.claude/status-config.json`
